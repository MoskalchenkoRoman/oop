        ...
        for (int i = 0; i < Main.TEAM_SIZE; i++) {
            ...
            if (Main.leftSide.get(i).hp != 0) {
                if (Main.rightSide.get(i).getPosition().isEquals(position)) {
                    ...
                    strL = " ".repeat(3) + AnsiColors.ANSI_GREEN + Main.leftSide.get(i).toString() + AnsiColors.ANSI_RESET;
                }
            } else {
                if (Main.rightSide.get(i).getPosition().isEquals(position)) {
                    str = "|" + AnsiColors.ANSI_RED + Main.rightSide.get(i).name.substring(0, 2) + AnsiColors.ANSI_RESET + "|";
                    strL = " ".repeat(3) + AnsiColors.ANSI_RED + Main.leftSide.get(i).toString() + AnsiColors.ANSI_RESET;
                }
            }
            if (Main.rightSide.get(i).hp != 0) {
                if (Main.rightSide.get(i).getPosition().isEquals(position)) {
                    str = "|" + AnsiColors.ANSI_BLUE + Main.rightSide.get(i).name.substring(0, 2) + AnsiColors.ANSI_RESET + "|";
                    strR = " ".repeat(5) + AnsiColors.ANSI_BLUE + Main.rightSide.get(i).toString() + AnsiColors.ANSI_RESET;
                }
            } else {
                if (Main.rightSide.get(i).getPosition().isEquals(position)) {
                    str = "|" + AnsiColors.ANSI_RED + Main.rightSide.get(i).name.substring(0, 2) + AnsiColors.ANSI_RESET + "|";
                    strR = " ".repeat(5) + AnsiColors.ANSI_RED + Main.rightSide.get(i).toString() + AnsiColors.ANSI_RESET;
                }
            }
        }
